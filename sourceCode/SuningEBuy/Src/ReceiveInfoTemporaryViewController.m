//
//  ReceiveInfoTemporaryViewController.m
//  SuningEBuy
//
...
#import "ReceiveInfoTemporaryViewController.h"
...
@interface ReceiveInfoTemporaryViewController () <ChooseCouponDelegate,ShipModeSelectViewDelegate>
{
...
@implementation ReceiveInfoTemporaryViewController
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if ([cellType isEqualToString:@"InvoiceTypeDetail_Cell"])    //发票详细信息
    {
        ...
        taitouLbl.hidden = NO;
        leixinLbl.frame = CGRectMake(15, 10, 290, 15);
        taitouLbl.frame = CGRectMake(15, 35, 290, 15);
        zhuyiLbl.frame = CGRectMake(15, 60, 290, 15);
        ...
        else if (YES == self.canUseEleInvoice && invoiceType == 5 && (NO == self.eleInvoiceIsDefault || isChangeInvoice))
        {
            leixinLbl.frame = CGRectMake(15, 20, 290, 15);
            zhuyiLbl.frame = CGRectMake(15, 50, 290, 15);
            taitouLbl.hidden = YES;
            leixinLbl.text = L(@"PFNoInvoice");
            zhuyiLbl.text = L(@"PFPaperInvoicePleaseAmend");
        }
        else
        {
...
- (void)checkCityChange
{
        ...
        (self.shipMode == ShipModeSelfTake && self.storeInfo.cityId.length && ![self.storeInfo.cityId isEqualToString:self.deliveryCityCode]))
    {
        ...
        self.submitBtn.enabled = NO;
        /* Fix Me*/
        __weak ReceiveInfoTemporaryViewController *weakSelf = self;
        [self displayOverFlowActivityView];
        ...
        [shopCart reOrderCheckOutWithCity:city cityName:cityName logic:self.buyNowLogic complete:^(BOOL isSuccess, ShopCartV2Service *service, ShopCartLogic *logic) {
            /* Fix Me*/
            ReceiveInfoTemporaryViewController *self = weakSelf;
            self.navigationItem.rightBarButtonItem.enabled = YES;
            self.submitBtn.enabled = YES;
            [self removeOverFlowActivityView];
            if (isSuccess)
